...
export const Product = z.object({
  ...
  name: z
    .string()
    .min(5, { message: errorMessages.min('Name', 0) })
    .max(50, { message: errorMessages.max('Name', 50) }),
  description: z
    .string()
    .min(20, { message: errorMessages.min('Description', 20) }),
  ...
  rating: z
    .number()
    .min(0, { message: errorMessages.min('Rating', 0) })
    .max(5, { message: errorMessages.max('Rating', 5) }),
  category: z
    .string()
    .min(3, { message: errorMessages.min('Category', 3) })
    .max(100, { message: errorMessages.max('Category', 100) }),
  stock_level: z
    .number()
    .min(0, { message: errorMessages.min('Stock Level', 0) }),
