...
export const Customer = z.object({
    ...
    first_name: z.string().max(50, {message: errorMessages.max('First Name', 50)}),
    last_name: z.string().max(50, {message: errorMessages.max('Last Name', 50)}),
    email: z.string().email({message: errorMessages.invalid('Email')}),
    password: z.string().min(5, {message: errorMessages.min('Password', 5)}).max(50, {message: errorMessages.max('Street', 50)}),
